        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          {submitted ? (
            <div className="bg-white rounded-2xl p-12 border border-gray-100 text-center">
              ...
              <button onClick={() => { setSubmitted(false); setForm({ name:'', email:'', phone:'', budget:'', preferredAreas:[], propertyTypes:[] }); }} className="bg-[#0A2647] text-white font-heading font-semibold px-6 py-3 rounded-lg hover:bg-[#0A2647]/90 transition-colors">
                Update Preferences
              </button>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="bg-white rounded-2xl p-8 md:p-12 border border-gray-100 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <input type="tel" required value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} placeholder="(555) 123-4567" className={inputClass} />
                </div>
              </div>
              <div>
                <label className={labelClass}>Investment Budget ($) *</label>
                <input type="text" required value={form.budget} onChange={(e) => setForm({ ...form, budget: e.target.value })} placeholder="e.g. $100,000 - $300,000" className={inputClass} />
              </div>
              <div>
