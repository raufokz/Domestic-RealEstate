...
export default function BuyerDocumentsPage() {
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/buyer/documents`, {
        method: "POST",
  ...
  async function handleDownload(doc: Document) {
    const token = readAuthToken();
    const res = await fetch(`${API_BASE}/buyer/documents/${doc.id}/download`, {
      headers: token ? { Authorization: `Bearer ${token}` } : undefined,
    });
    if (!res.ok) {
  ...
  async function handleDelete(id: number) {
    ...
    try {
      await apiDelete(`/buyer/documents/${id}`);
  ...
  return (
    <BuyerLayout title="Documents" subtitle="Your private transaction document vault.">
