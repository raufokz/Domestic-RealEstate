...
export default function BrokersPage() {
  ...
  const fetchBrokers = useCallback(async () => {
    try {
      setLoading(true);
      const data = await apiGet<{ data: Broker[] }>("/admin/users?role=broker");
  ...
  const filtered = brokers.filter((b) => {
    const matchSearch = b.name.toLowerCase().includes(search.toLowerCase()) || b.email.toLowerCase().includes(search.toLowerCase()) || (b.company || "").toLowerCase().includes(search.toLowerCase());
    const matchStatus = statusFilter === "All" || b.status === statusFilter;
    return matchSearch && matchStatus;
  });

  const openCreate = () => { setEditingBroker(null); setForm(emptyForm); setShowModal(true); setError(""); };
  const openEdit = (b: Broker) => { setEditingBroker(b); setForm({ name: b.name, email: b.email, phone: b.phone || "", company: b.company || "", status: b.status }); setShowModal(true); setError(""); };
  ...
  const handleSave = async () => {
    ...
    try {
      setSaving(true);
      if (editingBroker) {
        await apiPut(`/admin/users/${editingBroker.id}`, { ...form, role: "broker" });
      } else {
        await apiPost("/admin/users", { ...form, role: "broker" });
      }
      setShowModal(false);
      fetchBrokers();
    } catch (e: unknown) {
  ...
  return (
    <AdminLayout title="Broker Management">
      <div className="bg-white rounded-xl shadow-sm p-4 mb-6">
        <div className="flex flex-col sm:flex-row gap-4">
          <div className="flex-1">
            <input type="text" placeholder="Search brokers by name, email or company..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#C9A227] focus:border-transparent" />
          </div>
        ...
        {loading ? (
          ...
          <>
              ...
              <table className="w-full">
                <thead className="bg-[#0A2647] text-white">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left text-sm font-semibold">Active Listings</th>
                ...
                <tbody className="divide-y divide-gray-100">
                  {filtered.map((broker) => (
                    <tr key={broker.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-gray-600">{broker.company || "-"}</td>
                      <td className="px-4 py-3 text-gray-600 text-center">{broker.active_listings ?? 0}</td>
                      <td className="px-4 py-3">
                        <span className={`px-2.5 py-1 text-xs rounded-full ${statusColors[broker.status] || statusColors.Inactive}`}>{broker.status}</span>
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <button onClick={() => openEdit(broker)} className="text-[#C9A227] hover:text-[#0A2647] text-sm font-medium">Edit</button>
                          <button onClick={() => setDeleteConfirm(broker.id)} className="text-[#8B1E3F] hover:text-red-700 text-sm font-medium">Delete</button>
                        </div>
                      </td>
                    </tr>
                  ))}
            ...
            <div className="flex items-center justify-between px-4 py-3 border-t border-gray-200">
              <p className="text-sm text-gray-600">Showing {filtered.length} of {brokers.length} brokers</p>
            </div>
          </>
        )}
      </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full p-6">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold text-[#0A2647]">{editingBroker ? "Edit Broker" : "Add Broker"}</h3>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input type="text" value={form.company} onChange={(e) => setForm({ ...form, company: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#C9A227]" />
              </div>
              <div>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-sm w-full p-6">
            <h3 className="text-lg font-semibold text-[#0A2647] mb-2">Delete Broker</h3>
            <p className="text-gray-600 mb-4">Are you sure you want to delete this broker? This action cannot be undone.</p>
