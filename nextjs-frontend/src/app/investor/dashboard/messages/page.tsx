...
export default function InvestorMessagesPage() {
  ...
  const fetchConversations = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await apiGet<Conversation[]>("/investor/messages");
  ...
  useEffect(() => {
    ...
    apiGet<{ data: Message[] }>(`/investor/messages/${selectedId}`)
  ...
  async function handleSend() {
    ...
    setSending(true);
    try {
      await apiPost("/investor/messages", { conversationId: selectedId, content: newMessage });
      setNewMessage("");
      const res = await apiGet<{ data: Message[] }>(`/investor/messages/${selectedId}`);
  ...
  return (
    <InvestorLayout title="Messages" subtitle="Your enquiry threads with agents.">
