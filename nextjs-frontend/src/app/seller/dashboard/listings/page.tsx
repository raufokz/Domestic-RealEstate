...
const STATUS_TABS = ["All", "Active", "Pending", "Sold", "Rejected"];
...
export default function SellerListingsPage() {
  const { data, loading, error, refetch } = useFetch<{ data: Property[]; meta?: { total: number } }>("/seller/properties?per_page=50");
  ...
  const handleDelete = async (prop: Property) => {
    ...
    setBusyId(prop.id);
    try {
      await apiDelete(`/properties/${prop.id}`);
      success("Listing deleted.");
      refetch();
    } catch (e) {
      notifyError(e, "Could not delete this listing.");
    } finally {
      setBusyId(null);
    }
  };
    ...
    <SellerLayout title="My Listings" subtitle="Manage all your property listings in one place.">
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <Link href="/seller/dashboard/listings/new" className="bg-[#C9A227] text-[#0A2647] px-4 py-2 rounded-lg text-sm font-semibold hover:bg-[#b8911f] transition flex items-center gap-2 w-fit">
        ...
        {error ? (
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-12 text-center text-slate-400">Loading listings...</div>
        ) : filtered.length === 0 ? (
          <div className="bg-white rounded-xl border border-slate-200 p-12 text-center">
            ...
            <Link href="/seller/dashboard/listings/new" className="inline-block mt-4 px-6 py-2 bg-[#C9A227] text-[#0A2647] rounded-lg text-sm font-semibold hover:bg-[#b8911f] transition">
              Add New Listing
            </Link>
          </div>
        ) : (
                      ...
                      <tr key={prop.id} className="hover:bg-slate-50 transition">
                        <td className="px-5 py-4">
                          <div>
                            ...
                            <p className="text-slate-500 text-xs">{[prop.address, prop.city, prop.state].filter(Boolean).join(", ") || "—"}</p>
                          </div>
                        </td>
                        <td className="px-5 py-4 text-sm font-semibold text-[#0A2647]">{prop.price ? `$${prop.price.toLocaleString()}` : "—"}</td>
                        ...
                        <td className="px-5 py-4 text-sm text-slate-600">{prop.view_count || 0}</td>
                        <td className="px-5 py-4 text-right">
                          <div className="flex items-center justify-end gap-3 flex-wrap">
                            <Link href={`/seller/dashboard/listings/edit/${prop.id}`} className="text-sm text-[#C9A227] hover:text-[#0A2647] font-medium">
                              Edit
                            </Link>
                            {prop.slug && (
